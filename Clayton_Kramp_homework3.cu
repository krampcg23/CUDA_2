...
main (int argc, char **argv) {
  ...
  char input;
  bool yesno = false;
  cout << "Would you like to enable optimizing global memory access?" << endl;
  cout << "This will improve memory access but take more 'prep' time" << endl;
  cout << "Enter y or n" << endl;
  cin >> input;
  if (input == 'y' || input == 'Y') yesno = true;
  ...
  printf("Kernel Execution Time:  %3.5f ms \n", time);
